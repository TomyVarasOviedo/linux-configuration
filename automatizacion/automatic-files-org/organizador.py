...
configFile = open("/home/tomy/.config/automatizacion/automatic-files-org/config.json")
...
def clasificar_usuario(archivo: str) -> bool:
    ...
    for carpeta in dict.keys(user_dirs):
        if os.path.isdir(ruta + f"/{carpeta}"):
            ...
            if detectarNombreArchivo(archivoNombre + extension, user_dirs[carpeta]):
                # Si coincide unas de las palabras claves con el nombre del archivo
                mover_archivo(archivo, carpeta)
                return True
        else:
            os.mkdir(ruta + f"/{carpeta}")
            if detectarNombreArchivo(archivoNombre, user_dirs[carpeta]):
                # Si coincide unas de las palabras claves con el nombre del archivo
                mover_archivo(archivo, carpeta)
                return True
...
def detectarNombreArchivo(archivoNombre: str, coincidencias: list) -> bool:
    if any(palabra in archivoNombre for palabra in coincidencias):
        return True
    else:
        return False
